...
Probability theory, interpreted as a calculus for reasoning under uncertainty, is the common thread running through much of modern algorithmic trading and quantitative finance. This chapter develops the **Bayesian** perspective: one in which probabilities represent degrees of belief that are updated consistently as new evidence arrives. Starting from first principles—Cox's derivation of probability as an extension of logic—we build up a toolkit that spans statistical inference, machine learning, and sequential decision-making.

The chapter is organized as follows. The opening section establishes the foundations of Bayesian probability: Bayes' theorem, prior and posterior distributions, and methods for assigning prior probabilities when data is scarce. We then cover **Bayesian decision theory**, which links probabilistic beliefs to optimal actions via a loss function; this includes the connections between mean, median, and MAP estimators, and closes with **Thompson Sampling** as a principled strategy for exploration–exploitation in sequential decisions. The next sections address **Bayesian Machine Learning**: Bayesian learning and prediction, online updating, and model selection via Bayes factors. We then develop **Bayesian Linear Regression** in detail, including conjugate priors, the predictive distribution, and the connection to Gaussian Processes. The chapter continues with **Probabilistic Graphical Models** — Bayesian Networks and Markov Random Fields — which provide the language for {ref}`intro_causal` and {ref}`rfq_models`. It closes with **latent variable models** covering Gaussian Mixture Models, Hidden Markov Models, and the Kalman filter, with parameter estimation via the EM algorithm.

## Bayesian probability

The greek philosopher Aristotle formally introduced the field of Logic as the tool of deduction and therefore one of the main tools of scientific progress.  Logic allows to derive conclusions based on propositions that are taken for granted. Those propositions might have been themselves derived in a logical way, or might be principles that cannot be derived themselves, the starting building blocks of any theory or discipline. 

A fundamental principle of logic is that of the "excluded third", for which a given proposition can only be either true or false, but there is not a third option. Therefore, logic is based on the assumption of complete certainty about the propositions, both the premises and the conclusions. But most of our knowledge (if not all, as defended by empiricists like David Hume) is uncertain, therefore we need a tool that allows us to derive conclusions from uncertain premises. Under the so-called Bayesian school of probability, such a tool is probability theory, whose rules, that were inferred for the analysis of more specific toy cases like games of chance, we will see that turn out to apply to the wider case of reasoning under uncertainty.  

In the Bayesian interpretation, a probability is a *statement of our degree of belief in a proposition*. For instance, the proposition "Apple stock will close tomorrow at a higher price than today" is, from today's point of view, neither true nor false. The application of logic is limited to situations where we have already observed the price of Apple tomorrow, and therefore we know if the proposition is true or false. However, we might be interested in making decisions today based on our confidence in the proposition without having to wait to know its trueness: we might trigger different trading strategies whose profits are linked to Apple stock depending on the degree of confidence on the proposition. Such way of reasoning happens in all spheres of our lives, where we are always looking ahead to an uncertain future: from minor decisions like which clothes to wear on a given day, to major ones like which professional career to follow. 
...
Then, using Boolean logic and ordinary algebra, he found that consistency constrains these real numbers associated to our degree of belief to follow the rules of probability theory: 
...
where $\bar{X}$ denotes the proposition that X is false. Therefore, if we are bound to use the rules of logical reasoning, we can see that our degrees of belief can be rigorously mapped to probability theory. 
...
which is Bayes's theorem. We can also derive the marginalization rule:
...
So if $A$ is true, then $B$ is true. Let us discuss a few ways this syllogism is affected by uncertainty.
...
We can now maximize this Lagrangian with respect to each $p_i$, although out of symmetry we can see that any permutation $i \rightarrow j$ leaves the functional invariant, so the solution must be $p_1 = ... = p_N$ and using the constraint this means $p_i = 1/N$ which recovers De Moivre's rule. 

More interesting are cases where we have extra restrictions. For instance, let us assume that our prior knowledge includes a restriction on the mean of a random variable that takes values $x_1, ..., x_N$ under each outcome, so we know:
...
Let us consider a final case in which we add a second restriction on the second moment of the distribution of the random variable $x$, namely $\sum_i p_i x_i^2 = \bar{X_i^2}$. The restricted optimization target now reads:
...
One strength of Bayesian hypothesis testing is that it correctly captures many attributes attached to the way scientific research is normally conducted:
...
* Scientists that disagree on the validity of a hypothesis will score different prior probabilities to $H$. However, if predictions that are strongly attributed to this hypothesis (and weakly to alternative ones) are confirmed, their posterior probabilities will tend to converge since the scientist that attached a lower prior probability for $H$ will also attach naturally a lower probability to the prediction overall, $P(E)$, by virtue of Bayes' theorem:
...
where we have used the continuous version of the expression which suits better the problem of parameter estimation. Notice that in this case the decision is about the value of the parameters that we will use as estimator, and the state of the world is the actual value of the parameter. Depending on the loss function chosen, different estimators solve this equation. Let us discuss the most typical setups:

**Quadratic error**: $L(\theta, \hat{\theta}) = (\theta - \hat{\theta})^2$. This error is symmetric and penalizes larger errors more with respect to the correct value of the parameter. The penalty increases marginally for larger errors as well. The estimation equation corresponds to the mean squared error (MSE)  functional:
...
**Absolute error**: $L(\theta, \hat{\theta}) = |\theta - \hat{\theta}|$. This loss function also penalizes larger errors more and is symmetrical, but the marginal rate of growth of the penalty is constant, not increasing as in the quadratic case. The estimation equation corresponds to the mean absolute error (MAE) functional:
...
The decision framework introduced above assumes that the posterior distribution $P(\theta|I)$ is fixed when the decision is made. In many practical applications, however, the decision itself generates new observations that update the posterior: the dealer who quotes a price observes whether the client hits or misses, the clinician who prescribes a treatment observes the patient outcome, the algorithm that sets a bid price on an auction observes whether it won. This sequential structure introduces a fundamental **exploration–exploitation trade-off**: should the decision-maker commit to the currently best-known action (exploitation), or try less certain alternatives in order to gather information that may improve future decisions (exploration)?

A natural Bayesian approach to this trade-off is **Thompson Sampling** (also known as posterior sampling or probability matching). The idea is to sample the unknown model parameters from their current posterior and act as if those sampled parameters were the true ones:

1. Sample model parameters: $\theta_{\text{sampled}} \sim P(\theta | \mathcal{D}_n)$, where $\mathcal{D}_n$ is the dataset accumulated up to round $n$.
2. Choose the action that is optimal under the sampled parameters: $a^* = \arg\max_a \mathbb{E}[R | a, \theta_{\text{sampled}}]$.
3. Execute action $a^*$, observe outcome, and update the posterior: $P(\theta | \mathcal{D}_{n+1}) \propto P(\text{outcome} | a^*, \theta) P(\theta | \mathcal{D}_n)$.

**Exploitation** arises naturally because, most of the time, the sampled parameters will be close to the posterior mean — the current best estimate — and the optimal action under the mean is also the optimal action under a typical sample. **Exploration** arises because the posterior is not a point mass: occasionally the sampler will draw parameters from the tail of the posterior, temporarily favoring a less-tried action. As data accumulates and the posterior concentrates, exploration decays automatically without requiring a separate tuning schedule.

**Application: Bayesian demand estimation.** In the dynamic pricing setting described in {ref}`rfq_models`, a dealer continuously estimates a demand curve $D(\delta; w) = Q e^{-w_1 \delta}$ from historical price–demand pairs $\mathcal{D}_n = \{(\delta_i, d_i)\}_{i=1}^n$. Using log-linearization ($\log d_i = \log Q - w_1 \delta_i + \varepsilon_i$) and a Gaussian prior $w \sim \mathcal{N}(w_0, \Lambda_0)$, the posterior remains Gaussian (see {ref}`blr-section`):

$$w | \mathcal{D}_n, \sigma^2 \sim \mathcal{N}(\mu_n, \Lambda_n)$$

Thompson Sampling in this setting samples a weight vector from the posterior and uses it to compute the revenue-maximizing spread:

$$w_{\text{sampled}} \sim \mathcal{N}(\mu_n, \Lambda_n)$$
$$\delta^*_{\text{sampled}} = \arg\max_\delta \; \delta \cdot D(\delta; w_{\text{sampled}}) = \frac{1}{w_{1,\text{sampled}}}$$

where the last equality uses the closed-form solution for the exponential demand curve. The dealer quotes $\delta^*_{\text{sampled}}$, observes the client's response, and updates the posterior.

As the posterior $\mathcal{N}(\mu_n, \Lambda_n)$ narrows with more data, the sampled weights concentrate around $\mu_n$ and Thompson Sampling converges to pure exploitation. The trade-off is thus controlled automatically by the posterior uncertainty — a principled alternative to heuristic $\epsilon$-greedy or UCB (Upper Confidence Bound) strategies. Thompson Sampling is known to achieve asymptotically optimal regret (the cumulative loss from not always choosing the best action) in a broad class of sequential decision problems, making it an attractive choice whenever the Bayesian model is well-specified.
...
where we have set the restrictions over the expected value of the log-probabilities, i.e. $E[\log p_H] = \sum_i P(p_{H,i})\log p_{H,i}$ and $E[\log p_T] = \sum_i P(p_{H,i})\log(1 - p_{H,i})$. 
...
* As we saw in the previous section, in the particular case where we use conjugate priors, the posterior can be computed analytically. For a conjugate prior, the posterior belongs to the same family of distributions as the prior, with updated parameters.
...
$$L = \log p(\theta | {\it D}) \simeq L(\theta_{MAP}) + \frac{1}{2} \frac{d^2 L}{d\theta^2} \bigg|_{\theta_{MAP}} (\theta - \theta_{MAP})^2$$
...
$$ p(\theta | {\it D}) \simeq A \exp \left[ \frac{1}{2} \frac{d^2 L}{d\theta^2} \bigg|_{\theta_{MAP}} (\theta - \theta_{MAP})^2 \right] $$
...
$$\frac{d^2 L}{dp^2} \bigg|_{p_{MAP}} = - \frac{N + \alpha + \beta - 2}{p_{MAP} q_{MAP}}$$
...
We can test the validity of this result numerically by plotting the actual posterior and the Laplace approximation, for different numbers of observations. The following figure compares the two for a prior $\alpha = \beta = 2$ and the same simulated sequence used in the previous section:

```{figure} figures/BML_laplace_approximation.png
:name: fig:BML_laplace_approximation
:width: 8in
Exact Beta posterior (solid) and Laplace Gaussian approximation (dashed) for the coin toss experiment with prior $\alpha = \beta = 2$ and true $p_H = 0.3$, for increasing numbers of observations. The approximation improves markedly as $N$ grows and the posterior becomes more symmetric and concentrated.
```

As the figure illustrates, the Laplace approximation is poor with very few observations, when the posterior is still skewed and far from Gaussian. It improves steadily as more data arrives and the Beta posterior becomes more symmetric. Note also that both the exact prediction ($p_{MAP}$ for the Laplace case, $\bar{p}_H$ for the exact) and the approximation converge to the true value, consistent with the analysis above.

### Bayesian model selection

When it comes to comparing competing models or tuning hyperparameters, Bayesian Machine Learning differs fundamentally from other approaches. As noted by MacKay {cite:p}`mackay2003information`, the Bayesian approach has several key practical advantages: no held-out validation set is needed (all training data is used for both fitting and comparison), regularization constants can be optimised simultaneously with model parameters, the objective function is not noisy in contrast to cross-validation, and its gradient with respect to hyperparameters can be evaluated.

The mechanism is the following. Given two alternative models $H_1$ and $H_2$, Bayes' theorem gives:

$$P(H_i \mid D) = \frac{P(D \mid H_i)\, P(H_i)}{P(D)}$$

Taking the ratio of posteriors, and assuming equal prior probabilities $P(H_1) = P(H_2)$, the normalization $P(D)$ cancels:

$$\frac{P(H_1 \mid D)}{P(H_2 \mid D)} = \frac{P(D \mid H_1)}{P(D \mid H_2)} \equiv BF_{1,2}(D)$$

This ratio of *evidences* is called the **Bayes factor** $BF_{1,2}$. The evidence $P(D \mid H_i)$ is the normalizing constant of the posterior over the parameters of model $i$:

$$P(\boldsymbol{\theta} \mid D, H_i) = \frac{P(D \mid \boldsymbol{\theta}, H_i)\, P(\boldsymbol{\theta} \mid H_i)}{P(D \mid H_i)}$$

Computing evidences is hard in general, but the Laplace approximation leads to the **Bayesian Information Criterion (BIC)**:

$$BIC(H_i) \equiv \log L_i(\theta_{MAP,i}) - \frac{d_i}{2} \log N$$

where $d_i$ is the number of parameters in model $i$ and $N$ the number of observations. The log Bayes factor is then approximated as:

$$\log BF_{1,2}(D) \simeq BIC(H_1) - BIC(H_2)$$

This reveals the key property of Bayesian model selection: it naturally encodes the **bias-complexity tradeoff**. A higher-dimensional model ($d_i$ larger) pays a larger log-$N$ penalty, even if it achieves a higher likelihood, unless the improvement in log-likelihood justifies the extra complexity. The BIC (and the related Akaike Information Criterion, AIC) is used heuristically in classical Machine Learning to guard against overfitting; here it arises as a direct consequence of comparing posterior model probabilities. Following the Jeffreys scale, a Bayes factor above 10 is considered *strong evidence* for the favoured model, and below $1/10$ strong evidence against it.

#### Example: single coin vs two alternating coins

To illustrate model selection in action, we revisit the coin toss experiment and ask: were all observations generated by a single coin ($H_1$) or by two coins tossed alternately ($H_2$)? Assuming a Beta prior $\text{Beta}(\alpha, \beta)$ on each coin's probability, the evidences can be computed analytically using the conjugacy property:

$$P(D \mid H_1) = \frac{B(\alpha + N, \beta + N - n_H)}{B(\alpha, \beta)}$$

$$P(D \mid H_2) = \frac{B(\alpha + N_{\text{odd}},\, \beta + N_{\text{odd}} - n_{H,\text{odd}})\; B(\alpha + N_{\text{even}},\, \beta + N_{\text{even}} - n_{H,\text{even}})}{B(\alpha, \beta)^2}$$

where $N_{\text{odd}}, N_{\text{even}}$ are the numbers of odd- and even-indexed tosses and $n_{H,\text{odd}}, n_{H,\text{even}}$ the corresponding head counts. The Bayes factor is:

$$BF_{1,2}(D) = \frac{B(\alpha + N,\, \beta + N - n_H)\; B(\alpha, \beta)}{B(\alpha + N_{\text{odd}},\, \beta + N_{\text{odd}} - n_{H,\text{odd}})\; B(\alpha + N_{\text{even}},\, \beta + N_{\text{even}} - n_{H,\text{even}})}$$

When data are generated by a single coin ($p = 0.3$), the Bayes factor grows steadily above 1 as more observations confirm the simpler model, as shown in the following figure. In this case there is no complexity-fitness tradeoff favoring $H_2$ since the true generating model is the simpler one.

```{figure} figures/BML_bayes_factor_h1.png
:name: fig:BML_bayes_factor_h1
:width: 8in
Bayes factor $BF_{1,2}$ as a function of the number of observations when data are generated by a single coin ($H_1$, $p = 0.3$). Both the exact value and the BIC approximation grow well above the strong-evidence threshold of 10, correctly supporting $H_1$.
```

When the data instead come from two alternating coins ($p_1 = 0.3$, $p_2 = 0.7$), the Bayes factor first exceeds 1 — reflecting the parsimony advantage of $H_1$ with little data — but then collapses as the growing evidence for two different bias levels overwhelmingly favors $H_2$:

```{figure} figures/BML_bayes_factor_h2.png
:name: fig:BML_bayes_factor_h2
:width: 8in
Bayes factor $BF_{1,2}$ when data are generated by two alternating coins ($H_2$, $p_1 = 0.3$, $p_2 = 0.7$). With few observations $H_1$ is initially preferred for its lower complexity; with more data the evidence for $H_2$ becomes overwhelming, pushing $BF_{1,2}$ well below the strong-evidence threshold of $0.1$.
```

This interplay between parsimony and fit is the hallmark of proper Bayesian model selection, and stands in contrast to pure likelihood-based comparisons that would always favour the more complex model.

## Bayesian Linear Regression

The linear regression model is one of the most widely used in Machine Learning and statistics, owing to its simplicity, interpretability, and well-understood theoretical properties. In this section we revisit it from a Bayesian perspective.

### Linear Regression Model

As its name states, this is a regression model — the target is a continuous variable (possibly a vector) — where the relationship between the inputs $x_i$ and the target $y$ is linear. The model allows for a degree of misfitting between observations and the linear relationship by adding a noise term $\epsilon$:

$$y = a + \sum_i b_i x_i + \epsilon, \quad \epsilon \sim \mathcal{N}(0, \sigma^2)$$

where $a$ is a constant (the intercept) and $b_i$ are regression coefficients. For simplicity, we analyse the case where the output is one-dimensional and absorb the intercept by extending the feature vector with a constant feature. The likelihood of the data is then:

$$P(D \mid \{b_i\}, \sigma) = \prod_n \frac{1}{\sqrt{2\pi\sigma^2}}\, e^{-\frac{1}{2\sigma^2}(y_n - \sum_i b_i x_{i,n})^2}$$

Working with the negative log-likelihood:

$$-\log P(D \mid \{b_i\}, \sigma) = \frac{1}{2\sigma^2} \sum_n \left(y_n - \sum_i b_i x_{i,n}\right)^2 + \frac{N}{2} \log(2\pi\sigma^2)$$

Maximising the likelihood with respect to the regression coefficients is equivalent to minimising the mean squared error (MSE). The maximum likelihood (ML) estimators are:

$$\vec{\beta} = (X^t X)^{-1} X^t \vec{y}, \quad \hat{\sigma}^2 = \frac{1}{N}\sum_n (y_n - \hat{y}_n)^2$$

where $X$ is the matrix of feature observations. In the Machine Learning literature, two popular extensions address the risk of overfitting. **Ridge regression** adds a quadratic penalty on the coefficients:

$$ROF = \frac{1}{N}\sum_n \left(y_n - \sum_i b_i x_{i,n}\right)^2 + \lambda \sum_i b_i^2$$

which shrinks all coefficients towards zero while keeping them non-zero. **Lasso regression** uses instead a linear (absolute-value) penalty:

$$LOF = \frac{1}{N}\sum_n \left(y_n - \sum_i b_i x_{i,n}\right)^2 + \lambda \sum_i |b_i|$$

The $\ell_1$ penalty has the distinctive property that above a critical value of $\lambda$ the optimal solution sets the coefficients of the least relevant features to exactly zero, thereby performing automatic feature selection.

The following two figures illustrate these regularization paths on a synthetic dataset with $N=1000$ observations, 20 features of which only 10 are informative.

```{figure} figures/BLR_ridge_path.png
:name: fig:BLR_ridge_path
:width: 8in
Ridge regression coefficients as a function of the regularization strength $\lambda$. As $\lambda$ increases all coefficients are progressively suppressed but remain non-zero.
```

```{figure} figures/BLR_lasso_path.png
:name: fig:BLR_lasso_path
:width: 8in
Lasso regression coefficients as a function of the regularization strength $\lambda$. In contrast to Ridge, the Lasso sets the coefficients of the irrelevant features to exactly zero beyond a critical regularization strength, performing implicit feature selection.
```

(blr-section)=
### Bayesian Linear Regression Model

Bayesian linear regression treats the coefficients $\mathbf{b}$ (and possibly the noise variance $\sigma^2$) as random variables whose distribution is updated as new data arrive. Rather than returning a single point estimate, the Bayesian framework yields a full posterior distribution:

$$p(\mathbf{b}, \sigma^2 \mid D)$$

**Choice of prior.** Since the likelihood of the linear regression model is Gaussian, a Gaussian prior on the coefficients is the natural conjugate choice when $\sigma^2$ is known:

$$p(\mathbf{b}) \sim \mathcal{N}(\mathbf{b}_0, V_0)$$

After observing data $D = \{X, \mathbf{y}\}$, Bayes's theorem gives a posterior that is also Gaussian:

$$p(\mathbf{b} \mid X, \mathbf{y}, \sigma^2) \sim \mathcal{N}(\mathbf{b}_N, V_N)$$

$$\mathbf{b}_N = V_N V_0^{-1} \mathbf{b}_0 + \frac{1}{\sigma^2} V_N X^t \mathbf{y}$$

$$V_N = \sigma^2 (\sigma^2 V_0^{-1} + X^t X)^{-1}$$

Choosing a zero-mean prior $\mathbf{b}_0 = \mathbf{0}$ and $V_0 = \tau^2 I$, the MAP estimate of the weights becomes:

$$\mathbf{b}_N = \left(\frac{\sigma^2}{\tau^2} I + X^t X\right)^{-1} X^t \mathbf{y}$$

which is precisely the Ridge estimator with $\lambda = \sigma^2 / \tau^2$, since the mean and the mode of a Gaussian coincide. This formalises within the Bayesian framework a technique that was originally introduced as a heuristic. Similarly, a Laplace prior on the coefficients yields a MAP estimator equal to the Lasso estimator with $\lambda = 2\tau\sigma^2$.

When $\sigma^2$ is also unknown, the conjugate prior is the Normal-Inverse-Gamma (NIG) distribution:

$$p(\mathbf{b}, \sigma^2) = \text{NIG}(\mathbf{b}, \sigma^2 \mid \mathbf{b}_0, V_0, \alpha_0, \beta_0) \equiv \mathcal{N}(\mathbf{b} \mid \mathbf{b}_0, \sigma^2 V_0)\; \text{IG}(\sigma^2 \mid \alpha_0, \beta_0)$$

where $\text{IG}$ is the Inverse-Gamma distribution:

$$\text{IG}(\sigma^2 \mid \alpha_0, \beta_0) = \frac{\beta_0^{\alpha_0}}{\Gamma(\alpha_0)} \left(\frac{1}{\sigma^2}\right)^{\alpha_0+1} \exp\!\left(-\frac{\beta_0}{\sigma^2}\right)$$

The posterior is again NIG with updated parameters:

$$\mathbf{b}_N = V_N (V_0^{-1} \mathbf{b}_0 + X^t \mathbf{y}), \quad V_N = (V_0^{-1} + X^t X)^{-1}$$

$$\alpha_N = \alpha_0 + n/2, \quad \beta_N = \beta_0 + \frac{1}{2}\!\left(\mathbf{b}_0^t V_0^{-1} \mathbf{b}_0 - \mathbf{b}_N^t V_N^{-1} \mathbf{b}_N\right)$$

The marginal posteriors, obtained by integrating out the other parameter, are:

$$p(\sigma^2 \mid D) = \text{IG}(\alpha_N, \beta_N), \quad p(\mathbf{b} \mid D) = T\!\left(\mathbf{b}_N,\; \frac{\beta_N}{\alpha_N} V_N,\; 2\alpha_N\right)$$

where $T$ denotes the Student-$t$ distribution. Comparing the marginal for $\sigma^2$ with the least-squares estimator, we can identify $\alpha_N$ as an effective (half) number of observations and $\beta_N$ as a prior-corrected sum of squared residuals.

### Predictive Distribution

Prediction in the Bayesian framework integrates out the parameters using the posterior distribution. When $\sigma^2$ is known, integrating over the Gaussian posterior on $\mathbf{b}$ gives a Gaussian predictive distribution for a new feature vector $\mathbf{x}$:

$$p(y \mid \mathbf{x}, D, \sigma^2) = \int \mathcal{N}(y \mid \mathbf{x}^t \mathbf{b}, \sigma^2)\; \mathcal{N}(\mathbf{b} \mid \mathbf{b}_N, V_N)\; d\mathbf{b} = \mathcal{N}\!\left(y \mid \mathbf{b}_N^t \mathbf{x},\; \sigma_N^2(\mathbf{x})\right)$$

$$\sigma_N^2(\mathbf{x}) = \sigma^2 + \mathbf{x}^t V_N \mathbf{x}$$

The predictive variance has two additive contributions: the irreducible measurement noise $\sigma^2$, and the parameter uncertainty $\mathbf{x}^t V_N \mathbf{x}$, which is larger for inputs far from the training data. When $\sigma^2$ is also inferred from data, the predictive distribution becomes a Student-$t$:

$$p(y \mid \mathbf{x}, D) = T\!\left(y \;\Big|\; \mathbf{b}_N^t \mathbf{x},\; \frac{\beta_N}{\alpha_N}(1 + \mathbf{x}^t V_N \mathbf{x}),\; 2\alpha_N\right)$$

### Numerical Example

To visualise the Bayesian updates, we fit the NIG conjugate model incrementally on a one-feature synthetic dataset and sample ten regression lines from the posterior at each stage. With only a few observations the posterior is broad and many different regression lines are compatible with the data. As more data arrives, the posterior concentrates and all sampled lines converge towards a single one:

```{figure} figures/BLR_posterior_samples.png
:name: fig:BLR_posterior_samples
:width: 8in
Ten regression lines sampled from the posterior of a Bayesian linear regression model for increasing numbers of training observations ($N = 100$ to $N = 900$). With few data points the posterior is broad and the sampled lines are widely scattered; as $N$ grows they converge to a single estimate.
```

### Connection to Gaussian Processes

We have seen that when $\sigma^2$ is known and the prior on the coefficients is Gaussian, the posterior predictive distribution is a multivariate Gaussian. This can be recast by viewing the linear regression model as defining a family of random variables indexed by the inputs:

$$f(\mathbf{x}_i) = \mathbf{b}^t \mathbf{x}_i + \epsilon$$

Any finite collection of such variables follows a multivariate Gaussian distribution with:

$$E[f(\mathbf{x}_i) \mid D] = \mathbf{b}_N^t \mathbf{x}_i, \quad \text{cov}(f(\mathbf{x}_i), f(\mathbf{x}_j) \mid D) = (V_N)_{ij}\, x_i x_j + \sigma^2$$

This is a specific instance of a **Gaussian process** — a collection of (potentially infinitely many) random variables such that the joint distribution of any finite subset is multivariate Gaussian:

$$f(x) \sim \mathcal{GP}(\boldsymbol{\mu}(x),\, k(x, x'))$$

where $k(x, x')$ is called the kernel function. For Bayesian linear regression, the kernel takes the form:

$$k(x, x') = \sigma^2 + x^t V_N x'$$

Replacing the linear model with a non-linear feature map $\boldsymbol{\phi}(\mathbf{x})$ generalises this kernel to:

$$k(x, x') = \sigma^2 + \boldsymbol{\phi}(\mathbf{x})^t V_N \boldsymbol{\phi}(\mathbf{x}')$$

In modern Gaussian process modelling, the kernel is specified directly — encoding prior beliefs about the smoothness and structure of the function to be learnt — rather than derived from a basis expansion. A widely-used choice is the Radial Basis Function (RBF) kernel:

$$k(x, x') = \exp\!\left(-\gamma \| x - x' \|^2\right)$$

where $\gamma$ is a hyperparameter. Many classical stochastic processes are Gaussian processes with specific kernel functions: the Wiener process, the Ornstein–Uhlenbeck process, and the Brownian Bridge are notable examples. For a thorough treatment of Gaussian processes for Machine Learning, the classical reference is {cite:p}`rasmussen2006gaussian`.

## Probabilistic Graphical Models

Probabilistic graphical modelling is a branch of probabilistic modelling that exploits the mathematical and visual properties of graphs to simplify the tasks of sampling, inference, and parameter learning in models of the joint distribution of a set of random variables. Graphs allow us to introduce prior domain knowledge about the dependency structure of the variables, and when their structure is exploited it can dramatically simplify otherwise intractable calculations. The graph construction is in general a task for the researcher, though data can be used to falsify a proposed structure, and algorithms to infer structure from data are an active research area {cite:p}`koller2009probabilistic`.

### Graph Theory

Mathematically, a graph is a pair $G = (V, E)$ of vertices $V$ (nodes) and edges $E$ connecting them. Edges can be **directed** (arrows, representing causal or one-way relationships) or **undirected** (plain lines, representing symmetric dependencies or correlations). Directed graphs with no cycles are called **Directed Acyclic Graphs (DAGs)** and are the building block of Bayesian Networks. Edges can also carry weights representing the strength of the relationship.

A typical example of an undirected graph in financial markets is a representation of correlations between stock price returns. We use the IBEX 35 as an example: nodes are ten constituent stocks, and an edge is drawn between two stocks if their daily return correlation is statistically significant at the 1% level (F-test). The result is shown below:

```{figure} figures/PGM_ibex35_correlation.png
:name: fig:PGM_ibex35_correlation
:width: 8in
Undirected correlation graph of ten IBEX 35 constituent stocks, based on one year of daily return data. An edge is drawn between two stocks when their correlation is statistically significant at the 1% level.
```

The high degree of connectivity reflects the common exposure of IBEX 35 companies to Spanish and European macroeconomic conditions. We can complement this picture with a directed causal model that hypothesises that returns are driven by a Spanish macro factor and sector-specific factors (banking, construction, travel, energy):

```{figure} figures/PGM_ibex35_causal.png
:name: fig:PGM_ibex35_causal
:width: 8in
Directed causal graph for the same IBEX 35 stocks, with macro and sector factors as common parents. This DAG encodes the hypothesis that stock returns are driven by a Spanish macro factor and sector-specific effects.
```

### Markov Random Fields

The first family of probabilistic graphical models is **Markov Random Fields** (MRFs), also called undirected graphical models. Edges represent close dependencies between random variables, and the joint probability distribution factorises over the *maximum cliques* of the graph. A clique is a subset of nodes in which every pair of nodes is connected by an edge; a maximum clique is a clique that is not a subset of any larger clique. The factorisation reads:

$$P(\mathbf{x}) = \frac{1}{Z} \prod_{c \in \mathcal{C}_m} \phi(c)$$

where $\mathcal{C}_m$ is the set of all maximum cliques, $\phi(c)$ is a non-negative potential function (not necessarily normalised), and $Z$ is a normalisation constant. The figure below shows a simple six-node undirected graph whose three maximum cliques are $\{0,1,2\}$, $\{2,3\}$, and $\{3,4,5\}$. The joint distribution factorises as:

$$P(x_0, \ldots, x_5) = \frac{1}{Z}\, \phi_1(x_0, x_1, x_2)\, \phi_2(x_2, x_3)\, \phi_3(x_3, x_4, x_5)$$

```{figure} figures/PGM_mrf_example.png
:name: fig:PGM_mrf_example
:width: 6in
A six-node undirected graph with three maximum cliques $\{0,1,2\}$, $\{2,3\}$, and $\{3,4,5\}$. The joint probability distribution factorises into one potential function per maximum clique.
```

In the IBEX 35 correlation example, the clique structure of the empirical correlation graph determines how the joint distribution of daily returns can be factorised, reducing the number of free parameters relative to a fully unconstrained multivariate Gaussian. Such a factorised representation can improve the robustness of covariance estimation, support Monte Carlo sampling for scenario generation, or enable conditional inference such as computing expected returns for one stock given a known return for another.

### Bayesian Networks

The second family are **Bayesian Networks** (BNs), built upon DAGs. Nodes represent random variables and directed edges represent conditional distributions: for an edge $X \rightarrow Y$, the edge encodes $P(Y|X)$. The set of nodes with directed edges pointing into a given node $x_i$ are its *parents*, denoted $\text{Pa}(x_i)$. The joint probability distribution factorises as:

$$P(\mathbf{x}) = \prod_{i=1}^{N} P\!\left(x_i \mid \text{Pa}(x_i)\right)$$

As a concrete example, consider demand-driven pricing. Historical observations consist of prices $p$, demands $d$, and a client wealth proxy $W$ (e.g. a segment indicator). Demand is naturally driven by both price and wealth; additionally, the observed pricing policy may itself be influenced by the wealth of the client base, introducing an arrow $W \rightarrow p$. The resulting Bayesian Network and its joint factorisation are:

$$P(d, p, W) = P(d \mid p, W)\, P(p \mid W)\, P(W)$$

```{figure} figures/PGM_bn_pricing.png
:name: fig:PGM_bn_pricing
:width: 5in
Bayesian Network for demand-driven pricing. Demand $d$ is influenced by price $p$ and client wealth $W$; the observed pricing policy is also influenced by $W$, making it a confounder.
```

#### Rules of Conditional Independence — d-Separation

A key benefit of the Bayesian Network structure is the ability to read off conditional independence relationships directly from the graph without specifying the distributions. Two nodes are *d-separated* given a conditioning set $Z$ if every path connecting them is blocked. Three structural patterns govern blocking:

1. **Chain** $A \rightarrow C \rightarrow B$: when $C \in Z$, the path is blocked and $A \perp B \mid Z$.

2. **Confounder** $A \leftarrow C \rightarrow B$: when $C \notin Z$, $A$ and $B$ are d-connected (the common cause induces correlation). Conditioning on $C$ blocks the path and renders $A \perp B \mid Z$.

3. **Collider** $A \rightarrow C \leftarrow B$: when $C \notin Z$, the path is blocked and $A \perp B$. Conditioning on $C$ *opens* the path, making $A$ and $B$ d-connected (a non-intuitive effect: knowing the common effect induces a dependency between its causes).

The figure below illustrates the first two cases:

```{figure} figures/PGM_dsep.png
:name: fig:PGM_dsep
:width: 8in
Two d-separation examples. Left: in a directed chain, conditioning on an intermediate node (grey) blocks the path and d-separates the endpoints. Right: a confounder (temperature, grey) creates a spurious correlation between ice-cream sales and crime rate; conditioning on it blocks the indirect path.
```

These independence rules are important for pricing analysis. The quantity of interest for optimal pricing is $P(d|p,W)$ for each wealth segment, not the raw conditional $P(d|p)$. The latter mixes the direct price effect with the indirect effect mediated by the confounder $W$. The true demand curve (marginalised over segments) is:

$$P(d \mid p) = \sum_i P(d \mid p, W_i)\, P(W_i)$$

This differs from the naive application of the law of total probability, which would use $P(W_i|p)$ rather than $P(W_i)$, conflating the pricing policy with the structural price elasticity.

The d-separation rules discussed above are also the foundation of *causal inference*, which extends probabilistic graphical models to reason about the effect of active interventions rather than passive observations. This is a rich topic with important applications throughout the book — from demand modelling and pricing to strategy evaluation — and is covered in depth in the dedicated chapter {ref}`intro_causal`.
...
$$p(\text{bad}|x_n) = \frac{p(x_n|\text{bad}) p(\text{bad})}{p(x_n)} = \frac{ {\mathcal N}(x_n|\mu,\sigma_𝐵) (1-\pi_G)}{\pi_G  {\mathcal N}(x_n|\mu,\sigma_𝐺)+(1−\pi_G)  {\mathcal N}(x_n|\mu,\sigma_B)}$$
...
Since the latter are not observed, we need to integrate them out: 
...
$$Q(\theta|\theta^{(s)}) \equiv \mathbb{E}_s[\log \Pi^N_{n=1}P(x_n,z_n|\theta)] $$

$$=  \sum_n \sum_{z_n} P({\rm z_n}|{\rm x_n},\theta_{t})\log P(x_n,z_n|\theta) $$
...
For the M-step, we just need to substitute $q_{s+1}$ into the free energy:
...
* Convergence in the incomplete log-likelihood $l(\theta^{(s)})$, which we know from the previous proof that has to increase on each EM iteration. The only issue is that potentially might be computationally expensive to evaluate, since it requires the integration over the latent variables.
...
$$\|\theta^{(s+1)} - \theta^{(s)}\|_2 \leq \text{threshold}$$
...
$$\log\!\left[P(y_1) P(x_1|y_1) \prod_{t=2}^T P(y_t|y_{t-1}) P(x_t | y_t)\right] = \log P(y_1) + \log P(x_1|y_1) + \sum_{t=2}^T \left(\log P(y_t|y_{t-1}) + \log P(x_t | y_t)\right)$$
...
:width: 8in
Log-likelihood of the local level model evaluated with the estimation of the parameters using EM for each iteration. We see that log-likelihood never decreases, which is a sanity check for the correctness of the implementation.
```
...
:name: fig:local_level_em_smoothing
:width: 8in
