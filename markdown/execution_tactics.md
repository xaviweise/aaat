...
**Terminal condition.** If any quantity remains at $T$, it must be sold immediately via a market order. The forced liquidation incurs a cost of $b$ ticks per unit below mid-price:

$$\Pi_T^{\text{terminal}} = -b \, q_T, \qquad b > 0$$

The parameter $b$ is a **market property** — the effective half-spread plus market impact of an unplanned emergency order — estimated from historical trade data during the calibration period alongside $A$ and $k$. It is not a free parameter that the tactic designer controls.

**Objective.** The tactic maximises the expected CARA (Constant Absolute Risk Aversion) utility of total proceeds, with risk aversion coefficient $\gamma > 0$:

$$J(t, q) = \sup_{\{\delta_s\}_{s \geq t}} \mathbb{E}_t \!\left[-e^{-\gamma \Pi_T}\right]$$

where $\Pi_T = \sum_{\text{fills}} \delta_{\text{fill}} - b\,q_T$ is total proceeds including the terminal liquidation. The risk aversion parameter $\gamma$ is the tactic's aggressiveness control: a high-$\gamma$ agent dislikes the possibility of reaching the deadline with unsold inventory and posts aggressively to ensure execution; a low-$\gamma$ agent (near risk-neutral) prioritises price improvement and posts passively. This formulation is consistent with the exponential utility framework used in the market-making chapters.
...
$$H(t, q) \equiv -\frac{1}{\gamma}\ln\!\bigl(-J(t, q)\bigr)$$

The boundary conditions become $H(T, q) = -b\,q$ (terminal liquidation cost) and $H(t, 0) = 0$ (no inventory, no further proceeds).

### The HJB equation and optimal placement

Let $\Phi(t,q) \equiv -J(t,q)$ denote the minimised disutility ($\Phi > 0$, related to $H$ by $\Phi = e^{-\gamma H}$). Applying Bellman's principle over $[t, t+dt]$ and taking $dt \to 0$, the **Hamilton–Jacobi–Bellman equation** for $\Phi$ is:

$$0 = \partial_t \Phi + \min_{\delta \geq 0}\; Ae^{-k\delta}\!\left[e^{-\gamma\delta}\Phi(t,q-1) - \Phi(t,q)\right]$$

Setting the derivative with respect to $\delta$ to zero gives the first-order condition:

$$-k\!\left[e^{-\gamma\delta}\Phi_{q-1} - \Phi_q\right] - \gamma e^{-\gamma\delta}\Phi_{q-1} = 0 \qquad \Rightarrow \qquad (k+\gamma)\,e^{-\gamma\delta^*}\Phi_{q-1} = k\,\Phi_q$$

Converting to the certainty-equivalent $H$ (using $\Phi_q = e^{-\gamma H_q}$) and defining:

$$\Delta H(t, q) \equiv H(t, q) - H(t, q-1) \leq 0$$

the optimal placement depth is:

$$\boxed{\delta^*(t, q) = \frac{1}{\gamma}\ln\!\left(1 + \frac{\gamma}{k}\right) + \Delta H(t, q)}$$

The first term, $\frac{1}{\gamma}\ln(1+\gamma/k)$, is the **risk-comfort depth**: the base placement a risk-averse agent maintains even without inventory urgency. It replaces the $1/k$ of the risk-neutral case, and has three key properties:

- **Recovery**: as $\gamma \to 0$, $\frac{1}{\gamma}\ln(1+\gamma/k) \to 1/k$ — the risk-neutral result is recovered.
- **Monotonicity**: the risk-comfort depth is strictly decreasing in $\gamma$ — greater risk aversion always tightens the placement relative to the risk-neutral benchmark.
- **Limit**: as $\gamma \to \infty$, the risk-comfort depth $\to 0$ — an extremely risk-averse agent posts at the best quote regardless of price improvement.

The interpretation of $\Delta H$ is unchanged: **the optimal placement depth equals the risk-comfort depth minus the absolute urgency $|\Delta H(t,q)|$**. A market order is optimal when $\delta^*(t,q) < 0$, i.e., when urgency exceeds the risk-comfort depth.

Substituting the optimal $\delta^*$ back and transforming to $H$ gives the reduced ODE in remaining time $\tau = T - t$:

$$\partial_\tau H(\tau, q) = \underbrace{\frac{A}{k+\gamma}\!\left(\frac{k}{k+\gamma}\right)^{\!k/\gamma}}_{C_\gamma}\exp\!\left(-k\,\Delta H(\tau,q)\right)$$

The constant $C_\gamma$ encodes both the market fill-rate parameters $(A, k)$ and the agent's risk aversion $\gamma$. As $\gamma \to 0$, $C_\gamma \to A/(ek)$, recovering the risk-neutral ODE. The structure of the equation — and hence the backward induction algorithm — is identical to the risk-neutral case; only the driving constant changes.

**Analytical solution for $q = 1$.** When only one unit remains, $H(\tau, 0) = 0$, so $\Delta H(\tau, 1) = H(\tau, 1)$ and the ODE becomes:

$$\partial_\tau H(\tau, 1) = C_\gamma\, e^{-k H(\tau, 1)}$$

This separable ODE has the solution (with initial condition $H(0,1) = -b$):

$$H(\tau, 1) = \frac{1}{k}\ln\!\left(e^{-kb} + k\,C_\gamma\,\tau\right)$$

The optimal placement depth for the last unit is:

$$\delta^*(\tau, 1) = \frac{1}{\gamma}\ln\!\left(1+\frac{\gamma}{k}\right) + \frac{1}{k}\ln\!\left(e^{-kb} + k\,C_\gamma\,\tau\right)$$

As $\tau \to 0$: $\delta^*(0,1) = \frac{1}{\gamma}\ln(1+\gamma/k) - b$, which triggers a market order when $b > \frac{1}{\gamma}\ln(1+\gamma/k)$ — a condition that is increasingly easy to satisfy as $\gamma$ increases. As $\tau \to \infty$: $\delta^*(\tau,1)$ grows without bound — with ample time, even a risk-averse agent posts very passively. For $q > 1$, the system is solved numerically by backward induction on the grid $(\tau, q)$.
...
- **Market order region**: the shaded region where $\delta^* < 0$ corresponds to states where the risk of terminal liquidation is so imminent that immediate execution via a market order is optimal. This region grows with $\gamma$: a more risk-averse agent accepts market orders in more states.
...
$$\delta_j^*(t, q) = \frac{1}{\gamma}\ln\!\left(1+\frac{\gamma}{k}\right) + \Delta H(t, q) \quad \text{for all } j = 1, \ldots, m$$
...
$$\delta^{(k)*}(t, q) = \frac{1}{\gamma}\ln\!\left(1 + \frac{\gamma}{k_k}\right) + \Delta H(t, q), \quad k = 1, \ldots, K$$

The optimal depth at each venue follows the same formula as the single-venue case, with the risk-comfort depth $\frac{1}{\gamma}\ln(1+\gamma/k_k)$ being venue-specific through $k_k$. This has a direct economic interpretation: **venues where fill rates fall off slowly with depth (small $k_k$) attract deeper, more passive placements, while venues where fill rates fall off rapidly (large $k_k$) attract more aggressive placements**. Additionally, for a given $\gamma$, depth is tighter everywhere compared with the risk-neutral benchmark.

The substituted HJB becomes:

$$-\partial_t H(t,q) = \sum_{k=1}^K C_\gamma^{(k)} \exp\!\left(-k_k \, \Delta H(t,q)\right), \qquad C_\gamma^{(k)} = \frac{A_k}{k_k+\gamma}\!\left(\frac{k_k}{k_k+\gamma}\right)^{\!k_k/\gamma}$$
...
- **Reward** $r_t$: proceeds from any fill at step $t$, minus the terminal liquidation cost at $T$ (with $b$ the calibrated market parameter). Formally:

$$r_t = \begin{cases} a_t & \text{if a fill occurs at step } t \\ -b \, q_T & \text{if } t = T \text{ and } q_T > 0 \\ 0 & \text{otherwise} \end{cases}$$

This linear reward corresponds to the expected-proceeds (risk-neutral) objective — the $\gamma \to 0$ limit of the CARA criterion. The RL agent therefore learns to approximate the risk-neutral limit of the HJB policy. The theoretically grounded approach to risk-averse execution is to use the CARA HJB directly with an explicit $\gamma$, but the RL framework is valuable precisely when the state space is richer than $(\tau, q)$ and the fill rate model is non-stationary or complex.

The RL agent learns a policy $\pi_\theta(\mathbf{s})$ that maximises the expected discounted sum of rewards $\mathbb{E}[\sum_t \rho^t r_t]$ (where $\rho$ is the discount factor) by estimating $\pi$ from experience rather than from explicit HJB solution.
...
* Verify the analytical solution $H(\tau, 1) = k^{-1}\ln(e^{-kb} + kC_\gamma\tau)$, where $C_\gamma = \frac{A}{k+\gamma}\!\left(\frac{k}{k+\gamma}\right)^{k/\gamma}$, by substituting into the HJB equation $\partial_\tau H(\tau, 1) = C_\gamma e^{-kH(\tau,1)}$ and checking that the initial condition $H(0,1) = -b$ is satisfied. Show that as $\gamma \to 0$ the solution reduces to the risk-neutral form $k^{-1}\ln(e^{-kb} + A\tau/e)$. Interpret the limiting cases $\tau \to 0$ and $\tau \to \infty$ in terms of the optimal depth $\delta^*(\tau, 1) = \frac{1}{\gamma}\ln(1+\gamma/k) + k^{-1}\ln(e^{-kb} + kC_\gamma\tau)$, and describe how the market-order threshold at $\tau = 0$ depends on $\gamma$.
