...
- **Instrument Features (BF)**: Encompasses the instrument's unique identifier along with its financial attributes and liquidity characteristics. For bonds these include coupon, yield, maturity, and interest rate sensitivity (e.g., DV01); for interest rate swaps, the tenor, notional currency, and fixed rate; for FX products, the currency pair and settlement date; for options, the strike, expiry, and relevant Greeks. Liquidity attributes — such as market bid-ask spreads, volatility, and the typical number of active dealers — are relevant across all asset classes. For conciseness the label BF is retained throughout; the framework applies equally to any RfQ-traded instrument by substituting the relevant product attributes.
...
- $P(\text{axe} \mid \text{RfQ})$: the distribution of axed RfQs across instruments and clients.
- $P(\text{RfQ} \mid CF, BF)$: baseline client–instrument demand preferences, estimated via collaborative filtering or topic models (e.g., latent Dirichlet allocation on historical trading flows).
...
The core extraction task is straightforward to state: given a chat message or a short conversation thread, determine whether the client is requesting a quote, and if so, extract the fields required by the downstream pricing pipeline — instrument identifier (ISIN for bonds, currency pair and tenor for FX and rates, or equivalent for other asset classes), direction (buy or sell, pay or receive), notional, settlement date, and any special conditions. This is an instance of intent classification and named entity recognition (NER), tasks for which instruction-tuned LLMs are well suited. As discussed in {ref}`sec:gai_llm`, modern foundation models can perform zero-shot and few-shot structured extraction from free-form text, formatting the output directly as a JSON schema compatible with downstream systems, without the need for task-specific training data. Their flexibility is particularly valuable in this domain: market messages mix abbreviated tickers, asset-class conventions, Bloomberg-specific shorthand, and multilingual fragments in ways that defeat purpose-built NLP models trained on cleaner corpora.
...
flowchart TD
    ...
    KB["📚 Knowledge Base (RAG)\nInstrument fundamentals · Pricing guidelines"]
