...
$$
\lambda(t) = \mu + \sum_{t_i < t} \phi\, e^{-\beta(t - t_i)},
$$

where $\mu > 0$ is the baseline (background) intensity, $\phi \geq 0$ is the jump in intensity triggered by each event, and $\beta > 0$ is the decay rate. After an order arrives at time $t_i$, intensity jumps by $\phi$ and then decays exponentially back toward the baseline. The process is **stationary** when $\phi / \beta < 1$, which imposes a bound on the strength of self-excitation. The stationary mean intensity is $\bar{\lambda} = \mu\beta / (\beta - \phi)$ and the variance of the event count over a window $\tau$ is asymptotically $\bar{\lambda}\tau \cdot \beta^2/(\beta - \phi)^2$, the Poisson variance inflated by the self-excitation factor.
...
**Exercise 1 (Hawkes process stationarity):** Show that the stationary mean intensity of a univariate Hawkes process is $\bar{\lambda} = \mu\beta/(\beta - \phi)$, and derive the condition $\phi < \beta$ for stationarity. Simulate the process using the thinning algorithm for $(\mu, \phi, \beta) = (0.5, 1.5, 2.0)$ and verify empirically that the sample mean intensity approaches $\bar{\lambda}$.
