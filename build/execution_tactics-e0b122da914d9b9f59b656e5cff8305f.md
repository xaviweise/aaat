...
The fundamental uncertainty in passive execution is whether a resting limit order will be hit before the execution window closes. To model this, consider a sell limit order placed $\delta$ ticks above the current mid-price $S_t$. The order fills when either (i) the mid-price rises by at least $\delta$ ticks, or (ii) an incoming market buy order is large enough to consume liquidity up to and including that price level. From the perspective of the order-placer, both mechanisms produce the same observable outcome: the order fills at a random time.

Following the framework of Avellaneda and Stoikov {cite:p}`AvellanedaStoikov2008` and the empirical analysis of Cont, Stoikov, and Talreja {cite:p}`cont2010stochastic`, we model fill events as a Poisson process with depth-dependent intensity:
...
In practice, $A$ and $k$ are calibrated from LOB data, typically by regressing historical fill rates against placement depth and time, controlling for market conditions (spread, recent volatility, order flow imbalance). The parameters vary by asset class, time of day, and market regime:

- **Equities in continuous auction**: typical values $A \sim 0.5$–$2.0$ fills per minute at depth zero, $k \sim 1$–$3$ per tick.
- **Fixed income and FX**: markets with wider spreads and slower order flow have smaller $A$ and larger $k$ — fill rates fall off more slowly because even deep orders have non-negligible fill probability over longer horizons.
- **Intraday variation**: $A$ is high at the open and close (high volume), lower at midday. Incorporating time-of-day effects leads to $A(t)$ in the model.

```{figure} figures/tact_fill_probability.png
:name: fig:tact_fill_probability
:width: 10in
