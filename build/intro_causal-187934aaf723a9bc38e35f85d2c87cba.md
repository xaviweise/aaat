...
1. **d-separation.** Consider the DAG with nodes $\{A, B, C, D, E\}$ and edges $A \to B$, $A \to C$, $B \to D$, $C \to D$, $D \to E$. (a) Is $A$ d-separated from $E$ given $\{D\}$? (b) Is $B$ d-separated from $C$ given $\{A\}$? (c) Is $B$ d-separated from $C$ given $\emptyset$? Justify each answer by identifying whether every path between the two nodes is blocked.

2. **Back-door criterion.** In a causal DAG representing the RfQ hit probability, the main variables are: spread quoted by dealer $\delta$, hit indicator $H$, market volatility $\sigma$, reference price flag $RF$, and a confounder $CF$ that affects both dealer pricing and client behaviour. The edges are $\sigma \to \delta$, $\sigma \to H$, $\delta \to H$, $RF \to H$, $CF \to \delta$, $CF \to H$. (a) Identify all back-door paths from $\delta$ to $H$ (paths that enter $\delta$ through a non-descendant). (b) Show that the adjustment set $\{\sigma, RF, CF\}$ satisfies the back-door criterion. (c) Write the back-door adjustment formula for $P(H \mid \text{do}(\delta))$.

3. **Interventional distribution via Rule 2.** In a simple chain DAG $Z \to X \to Y$ with an additional direct edge $Z \to Y$, and suppose there are no hidden confounders. (a) Write the observational factorisation $P(X, Y, Z)$. (b) Using Rule 2 of the do-calculus (action/observation exchange), show that $P(Y \mid \text{do}(X=x)) = \sum_z P(Y \mid X=x, Z=z) P(Z=z)$. (c) Verify that this is precisely the back-door adjustment with adjustment set $\{Z\}$.

4. **Counterfactual reasoning.** A dealer quotes $\delta' = 8$ bps and the client misses ($H = 0$). The SCM for the hit event is $H = \mathbf{1}[\delta \leq \delta_{\text{res}}]$, where the client's reservation spread is drawn as $\delta_{\text{res}} = U \cdot 20$ with $U \sim \text{Uniform}[0, 1]$ (the exogenous noise). (a) Compute $P(H = 0 \mid \delta' = 8)$. (b) Apply the abduction step: compute the posterior $P(U \mid H=0, \delta'=8)$. (c) Apply action and prediction: compute the counterfactual probability $P(H_{\delta=5} = 1 \mid H=0, \delta'=8)$. (d) Compare this to the interventional quantity $P(H=1 \mid \text{do}(\delta=5))$ and explain the difference.
