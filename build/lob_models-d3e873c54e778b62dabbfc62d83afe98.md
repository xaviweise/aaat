...
Order arrival as poisson, but then also include potentially hawkes, cox, etc to include patters from real markets. Review literature for stylized facts of order arrival and best distribution to fit them. Order arrival includes also type of order (we could have separate models for each type of orders, or select type after arrival, if statistics of arrival are similar), side (buy or sell) and size

### Visible and hidden orders

Models to infer hidden liquidity from lob data

## Market impact models

Traditional square root, meta orders, more advanced models etc

## Volume prediction models

volume prediction typically used in vwaps: prediction of next bin, prediction of static curve per day, total volume day prediction. Do research but also use KB. Link with the chapter on optimal execution, move here discussions on volume prediction if done in that chapter

A very simple prediction model is static volume curve computed with historical data. This can use standard machine learning models, and even simple medians or means over historical days discriminating potentially by day of week, special days (eg fed rates, ecb rates, expiration of options, payrolls in the usa, etc). Better models incorporate this as features. These models predict the full curve ahead

Dynamic models use intrady context to do the prediction. A simple but effective model is knn regression, just using similar days (also discriminating potentially by features like day of week etc as the static case). For large datasets knn can be optimized for faster retrieval, review literature on accelerating knn retrieval and add it to the machine learning chapter (all the knn if not added, include section in that chapter)

Explore literature on more advanced prediction models, i think vwap cited papers include some references on volume predictio, eg bussetti etc

## Probability of filling a limit order

Simple model distance to mid exponential (avellaneda stoikov)

literature on more complex models
- add more features
- models that work better, but need to respect monotonicity for optimization applications

Research papers like jp morgan research on advanced lob features using advanced ML. There are i think papers using the transformer architecture as well

It might make sense to generate a specific section on lob features that are used in other

## Short term price prediction

These are models that try to predict the direction of movement of prices in a short term (eg seconds or even less). Research literature. Typically Machine Learning models using features like order imbalance, rate of orders, volatility, etc. Link with LOB features section that we should definitely build

## Information Asymmetry

These are models mainly those from Marcos Lopez de Prado that infer "information asymmetry" from the observed lob data. Research literature and provide description of core models
